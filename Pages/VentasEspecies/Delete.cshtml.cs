...
namespace AtlasViewer.Pages.VentasEspecies
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        public DeleteModel(IVentaEspecieService ventaEspecieService, IEspecieService especieService)
        {
        ...
        public async Task<IActionResult> OnPostAsync(string id)
        {
            ...
            // Restaurar inventarios antes de eliminar
            foreach (var detalle in VentaEspecie.detalle)
            {
                var especie = await _especieService.GetByIdAsync(detalle.especieId!);
                if (especie != null)
                {
                    especie.inventarioAcumuladoKg += detalle.cantidadKg;
                    await _especieService.UpdateAsync(especie.Id!, especie);
                }
            }

            // Eliminar la venta
            await _ventaEspecieService.DeleteAsync(id);

            AlertService.Success(TempData, "Venta eliminada exitosamente. El inventario ha sido restaurado.");
            return RedirectToPage("./Index");
        }
    }
}
